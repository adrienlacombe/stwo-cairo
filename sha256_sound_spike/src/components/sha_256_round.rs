...
//
// SOUNDNESS-CRITICAL — ported from the decoded upstream reference
// spike/sha256-air/pr1425-ref/components/sha_256_round.rs
// (StarkWare stwo-cairo PR #1425 "sha256 builtin", OPEN/unmerged upstream).
//
// Mechanical changes vs the decoded reference (everything else is line-identical):
//   1. CairoSerialize/CairoDeserialize derives stripped (no proof-wire serialization in
//      this spike; the local prelude has no stwo-cairo-serialize).
//   2. The `constraints_regression_test_values::SHA_256_ROUND` regression test is removed
//      (that constant only exists in the upstream generated tree; it must be regenerated
//      when this component is integrated into a cairo-air pin).
//   3. UPSTREAM SOUNDNESS FLAG — `bind_ch_maj_limbs` + four binding constraints added:
//      in the decoded reference, `ch_limb_{0,1}_col{78,79}` and `maj_limb_{0,1}_col{114,115}`
//      are fed into the T1/T2 TripleSum32 adds but are NEVER constrained to the recombined
//      `chl/chh_col{76,77}` and `majl/majh_col{112,113}` columns (the honest witness fills
//      them as duplicates — see pr1425-ref/witness/sha_256_round.rs:538-546). As decoded,
//      a malicious prover can substitute arbitrary Ch/Maj values and still satisfy every
//      round constraint (demonstrated by `forged_ch_accepted_without_binding_constraints`
//      in tests/round_air.rs). Whether the real PR #1425 has these constraints elsewhere
//      (or the decode dropped them) MUST be verified against the upstream diff during
//      sign-off. Run with `bind_ch_maj_limbs = true` for the sound behavior; `false`
//      reproduces the decoded reference verbatim.
...
pub struct Eval {
    pub claim: Claim,
    /// SPIKE DEVIATION (see file header, item 3): when true, adds the four constraints
    /// binding ch_limb/maj_limb columns to their recombined chl/chh/majl/majh columns.
    /// `false` reproduces the decoded PR #1425 reference verbatim (underconstrained).
...
impl FrameworkEval for Eval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        // SPIKE DEVIATION vs decoded PR #1425 (file header, item 3): bind the ch_limb
        // columns consumed by the T1 add to the recombined chl/chh columns. Without these,
        // cols 78/79 are free and Ch(e,f,g) is forgeable.
        if self.bind_ch_maj_limbs {
        ...
        // SPIKE DEVIATION vs decoded PR #1425 (file header, item 3): bind the maj_limb
        // columns consumed by the T2 add to the recombined majl/majh columns.
        if self.bind_ch_maj_limbs {
...
// NOTE: the upstream `sha_256_round_constraints_regression` test is intentionally not
// ported — its pinned `SHA_256_ROUND` regression constant lives only in the upstream
// generated tree and must be regenerated at integration time. Behavioural accept/reject
// tests for this component live in tests/round_air.rs.
